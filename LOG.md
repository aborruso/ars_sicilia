...
- `extract_odg_data.sh`: il sanity check "PDF senza disegni estratti" ora guarda solo i PDF elaborati nel run corrente e non tutto lo storico. Gli OdG 269 e 270 (4-5 agosto) non hanno la sezione `DISCUSSIONE DEI DISEGNI DI LEGGE` — solo comunicazioni e interrogazioni, verificato con `markitdown` e `pdftotext` — quindi zero record è corretto e comparivano come falso allarme a ogni esecuzione.
- `extract_odg_data.sh`: negli mlr di normalizzazione gli escape `\x{0022}` `\x{201C}` `\x{201D}` `\x{2018}` `\x{2019}` sostituiti dai caratteri letterali. Il lexer di Miller 6.20 non riconosce più `\x{...}` e lo script si spezzava in locale; la CI installa 6.11 da Ubuntu noble e non lo vedeva. Verificato su entrambe le versioni: vecchia e nuova espressione danno output identico su 6.11, la nuova dà lo stesso output su 6.11 e 6.20, `disegni_legge.jsonl` invariato (237 record).
