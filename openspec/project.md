...
- Bash wrapper (`run_daily.sh`) for cron with lock handling
...
### Testing Strategy
- Manual smoke scripts: `test_youtube_auth.py` / `test_youtube_auth_manual.py` to validate OAuth tokens and quota.
- OAuth helper scripts: `get_auth_url.py` / `complete_auth.py` for manual authentication flow.
- Functional verification by running `build_anagrafica.py` (catalog only) and `main.py [seduta_url]` for end-to-end upload.
